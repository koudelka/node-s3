...
S3ConnectionPool.prototype.close_connections = function(bucket) {
  var connections_to_close
  if (bucket) {
    console.log('Closing connections to bucket: ' + bucket)
    connections_to_close = this.connections[bucket]
  }
  else {
    console.log('Closing connections to buckets: ' + _(this.connections).keys())
    connections_to_close = _(this.connections).chain().values().flatten()
  }

  connections_to_close.forEach(function(http_client) {
    if (http_client.parser)
      http_client.parser.incoming.connection.end()
  })
}
